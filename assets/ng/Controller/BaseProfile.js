(function(angular) {
	...
	angular
		.module('BaseProfile', [
			'BaseProfileDirectives',
			'JadwalDirectives',
			'KemajuanStudiDirective',
			'MahasiswaWaliDirective'
		])
		.controller('BaseProfileController', function(
			$scope,
			BaseProfileService,
			Jadwal,
			KhsmService,
			...
			BaseProfileService.get().then((response) => {
				if (response.Role == 'Mahasiswa') {
					...
					Jadwal.getJadwalKuliah($scope.DatasHome).then(
						(response) => {
							$scope.DatasJadwal = response;
						},
						(error) => {}
					);
					KhsmService.get($scope.Input).then(
						(response) => {
							$scope.Khsm.Datas = [];
							var b = response.data;
					...
					WaliMahasiswa.getList().then((response) => {
						...
						if (
							parseInt($scope.ListMonitoring.data[0].SMT_Tempuh) <= 4 ||
							(parseInt($scope.ListMonitoring.data[0].SKS_Lulus) < 30 &&
								parseInt($scope.ListMonitoring.data[0].IPK < 1))
						) {
							$scope.peringatan = $scope.ListMonitoring.pesan + ' 1';
						} else if (
							parseInt($scope.ListMonitoring.data[0].SMT_Tempuh) <= 6 &&
							parseInt($scope.ListMonitoring.data[0].SKS_Lulus) < 50
						) {
							$scope.peringatan = $scope.ListMonitoring.pesan + ' 2';
						} else if (
							parseInt($scope.ListMonitoring.data[0].SMT_Tempuh) <= 8 ||
							(parseInt($scope.ListMonitoring.data[0].SKS_Lulus) < 700 &&
								parseInt($scope.ListMonitoring.data[0].IPK < 2))
						) {
							$scope.peringatan = $scope.ListMonitoring.pesan + ' 3';
						} else if (
							parseInt($scope.ListMonitoring.data[0].SMT_Tempuh) <= 10 &&
							parseInt($scope.ListMonitoring.data[0].SKS_Lulus) < 90
						) {
							$scope.peringatan = $scope.ListMonitoring.pesan + ' TERAKHIR';
						} else if (
							parseInt($scope.ListMonitoring.data[0].SMT_Tempuh) <= 12 &&
							parseInt($scope.ListMonitoring.data[0].SKS_Lulus) < 100
						) {
							$scope.peringatan = $scope.ListMonitoring.pesan + ' TERAKHIR';
						}
					});
			...
			$scope.mahasiswaKelas = (item) => {
				Jadwal.getMahasiswakelas(item).then((x) => {
					$scope.datamhs = x;
